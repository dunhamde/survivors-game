...
func _test_grunt() -> void:
	...
	_ok(anim.hit_cells.size() == 1 and anim.hit_cells_south.size() == 1, "two hit poses")
	anim.set_facing_from_vector(Vector2.UP)
	_ok(anim.dir_col == 0, "N col 0 uses north clip")
	anim.trigger_hit()
	_ok(anim.hitting and anim.last_col == 0 and anim.last_row == 9, "north hit (0,9)")
	anim.start_death()
	...
	_ok(anim.dir_col == 4, "S col 4 uses south clip")
	anim.trigger_hit()
	_ok(anim.last_col == 1 and anim.last_row == 9, "south hit (1,9)")
	anim.start_death()
	anim.show_death_frame(0)
	_ok(anim.last_col == 1 and anim.last_row == 9, "south death starts on hit (1,9)")
	anim.show_death_frame(1)
