...
var _active_shields: Array[Area2D] = []
...
func _physics_process(delta: float) -> void:
	...
	for i in range(_active_shields.size() - 1, -1, -1):
		if not is_instance_valid(_active_shields[i]):
			_active_shields.remove_at(i)
	cooldown = maxf(0.0, cooldown - delta)
	if cooldown > 0.0:
		return
	var slots := maxi(1, data.base_count + _command_bonus()) - _active_shields.size()
	if slots <= 0:
		return
	var targets := nearest_targets(slots, 420.0)
	if targets.is_empty():
		return
